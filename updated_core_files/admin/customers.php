...
<?php
    ...
    $new_fields=', c.customers_telephone, a.entry_company, a.entry_street_address, a.entry_city, a.entry_postcode, c.customers_authorization, c.customers_referral, c.master_account';
  ...
  switch ($action) {
    ...
    default:
      ...
      if (isset($cInfo) && is_object($cInfo)) {
        ...
        // Handle master value 
        if ($cInfo->master_account != 0) { 
           $contents[] = array('text' => '<br>' . TEXT_CUSTOMER_NOT_MASTER); 
           $master_email = get_customers_email($cInfo->master_account); 
           $contents[] = array('text' => TEXT_CUSTOMER_TRUE_MASTER . $master_email . ' ' . '<a href='.  zen_href_link(FILENAME_CUSTOMERS, 'cID=' . (int)$cInfo->master_account, 'NONSSL') . '>' . 'Link' . '</a>');
           $lifetime_value = get_lifetime_value ($master_email, $cInfo->master_account); 
           $contents[] = array('text' => TEXT_CUSTOMER_MASTER_LIFETIME_VALUE  . $currencies->format($lifetime_value)); 
        } else { 
           $lifetime_value = get_lifetime_value ($cInfo->customers_email_address, $cInfo->customers_id); 
           $contents[] = array('text' => '<br />' . TEXT_CUSTOMER_MASTER_LIFETIME_VALUE  . $currencies->format($lifetime_value)); 
        }
